...
# Config
# ---------------------------------------------------------------------------
# Yangi tokeningiz muvaffaqiyatli joylashtirildi:
...
def send_question_edit(chat_id: int, message_id: int, state: dict) -> None:
    ...
    # Yangi savol uchun taymerni boshlash
...
def handle_unit_selection(message: types.Message) -> None:
    ...
    if ATTEMPT_LIMIT is not None:
        used = user_attempts.get(user_id, 0)
        if used >= ATTEMPT_LIMIT:
            bot.send_message(chat_id, f"Siz <b>{unit}</b> uchun maksimal urinishlar ({ATTEMPT_LIMIT}) soniga yetdingiz.")
            send_main_menu(chat_id)
            return

    user_attempts[user_id] = user_attempts.get(user_id, 0) + 1
    attempt_num = user_attempts[user_id]
    ...
    bot.send_message(chat_id, f"<b>{unit}</b> testi boshlanmoqda... Har bir savolga 8 soniya beriladi! 🚀", reply_markup=types.ReplyKeyboardRemove())
